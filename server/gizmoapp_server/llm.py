...
def _validate_messages(messages: Sequence[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for message in messages:
        ...
        if role not in ALLOWED_ROLES or not (valid_text or valid_content_parts):
            raise CourseLLMError(
                "each message must have a system, user, or assistant role and non-empty content."
            )
...
def ask_with_image(
    ...
    """Send text and an uploaded image to the course model."""
    if not isinstance(prompt, str) or not prompt.strip():
        raise CourseLLMError("prompt must be non-empty text.")
    if not isinstance(image, bytes) or not image:
        raise CourseLLMError("image must be non-empty bytes.")
    if image_content_type not in {"image/jpeg", "image/png", "image/webp"}:
        raise CourseLLMError("image_content_type must be a supported image type.")
